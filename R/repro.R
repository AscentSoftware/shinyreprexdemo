...
S7::method(repro, S7::class_any) <- function(x, ..., env = rlang::caller_env()) {
  x
}

S7::method(repro, class_reactive) <- function(x, ..., repro_code = Repro, init = TRUE, env = rlang::caller_env()) {
  ...
  reactive_calls <- lapply(reactive_exprs, \(x) repro(x, repro_code = repro_code, env = module_env))
...
S7::method(repro, class_calls) <- function(x, ..., repro_code = Repro, env = rlang::caller_env()) {
  ...
  if (is_input_call(x)) {
    ...
    eval_args <- repro(env[[rlang::call_name(x)]], repro_code = repro_code, init = FALSE)
    ...
    eval_args <- lapply(rlang::call_args(x), \(x) repro(x, repro_code = repro_code, init = FALSE, env = env))
